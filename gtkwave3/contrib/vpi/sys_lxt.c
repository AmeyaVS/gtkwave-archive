...
variable_cb_rosync (p_cb_data cause)
{
  ...
  while (a_info)
      ...
      if (vpi_get (vpiType, a_info->item) == vpiRealVar)
	{
	  ...
	  lt_emit_value_double (lt, a_info->ltsym, a_info->row,
				    value.value.real);
	}
      else
#endif
	{
	  ...
	  lt_emit_value_bit_string (lt, a_info->ltsym, a_info->row,
					value.value.str);
	}

      a_info->is_changed = 0;

#ifdef LXT_UNREGISTER
      {
	struct t_cb_data cb;
...
open_dumpfile (void)
{
  ...
  if (dump_path == NULL)
    {
      dump_path = strdup ("dump.lt");
    }

  lt = lt_init (dump_path);
...
draw_module_type (vpiHandle item, int typ, char *str)
{
  ...
  while ((net = vpi_scan (iter)))
...
#ifdef REALS_SUPPORTED
	  info->ltsym =
	    lt_symbol_add (lt, name, 0, 0, 0, LT_SYM_F_DOUBLE);
#else
	  free (info);
	  continue;
#endif
	...
	{
	  info->ltsym =
	    lt_symbol_add (lt, name, 0, info->lrange, info->rrange,
			       LT_SYM_F_BITS);
	}
...
static int
draw_scope_lt (vpiHandle item, int depth, int depth_max)
{
  vpiHandle orig = item;
  const char *name;
  char *type;
  ...
  if (depth == 0)
    {
      ...
      draw_module (item);
      draw_scope_lt (item, depth + 1, depth_max);
    }
  else
    {
      int i;
...
/*
 * This function is also used in sys_lt to check the arguments of the
 * lt variant of $dumpvars.
 */
static int
...
sys_dumpvars_calltf (char *name)
{
  ...
  for (; item; item = argv ? vpi_scan (argv) : 0x0)
    {
      int dep = draw_scope_lt (item, 0, depth);
    }
