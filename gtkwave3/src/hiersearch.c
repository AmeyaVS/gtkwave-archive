...
static void entrybox_local(char *title, int width, char *default_text, int maxch, GtkSignalFunc func)
{
    ...
    gtk_signal_connect(GTK_OBJECT(entry), "activate",GTK_SIGNAL_FUNC(enter_callback_e),entry);
    ...
    gtk_signal_connect(GTK_OBJECT (button1), "clicked",GTK_SIGNAL_FUNC(enter_callback_e),NULL);
    ...
    gtk_signal_connect(GTK_OBJECT (button2), "clicked",GTK_SIGNAL_FUNC(destroy_callback_e),NULL);
