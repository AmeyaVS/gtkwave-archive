 ...
 tok_next:
    ...
    tok_identifier:
        ...
        if (t < TOK_UIDENT)
            expect("identifier");
        s = sym_find(t); //KERMIN
        // Should print out indentifier usage here.
        printf("IDENT:%d:%s:%d:%s\n",t,file->filename,file->line_num,get_tok_str(t,NULL));    
...
static void decl(int l)
{
    ...
    while (1) {
		...
		printf("EXTERN:");
              ...
              else if(type.t & VT_STATIC){ 
		printf("STATIC:");
	      }
              else{ // Something slightly wrong... we'll have to rely on the data dump
		printf("LOCAL:");
	      }
    
              printf("%s:%s\n",get_tok_str(v,NULL),buffer); 
            ...
            if (tok == '{') {
                ...
                if (sym) {
                    ...
                    /* put function symbol */
	   	    sym = global_identifier_push(v, type.t, 0); // This could be important too
                    sym->type.ref = type.ref;
                }
		    ...
		    sym = sym_push(v, &type, 0, 0); // KERMIN - this is the critical sym_push 
